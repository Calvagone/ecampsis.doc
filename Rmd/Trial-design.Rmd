...
Covariates associated with subjects in the arm are defined, and optional dose‑adaptation rules based on these covariates can be specified (e.g. AMT * BW). 
...
By clicking on **add** button, a covariate editor window opens.
...
By clicking on **Bootstrap**, a dedicated window allows covariates to be sampled from an external dataset. Data can be entered directly into the table, imported from a CSV file using the **Browse** button, or retrieved from the NHANES database via the **Import DB** button.  Once the table is created, it can also be exported using the **Download** button.

![](pictures/bootstrap.png)

Within this interface, users can specify the covariates to include (e.g. AGE, BW) and preview the bootstrap dataset in a table. The Show filters option allows filtering the dataset by covariate values to refine the selection.

![](pictures/bootstrap2.png)

Two sampling options are available: Sample with replacement and Draw subjects randomly.
