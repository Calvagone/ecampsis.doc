...
getPDDescription <- function(x) {
  desc <- ""
  
  if (grepl("^direct_effect", x)) {
    desc <- "Direct effect PD model (Emax relationship driven directly by PK concentration)"
    
  } else if (grepl("^effect_cmt", x)) {
    desc <- "Effect compartment PD model with a delay between PK concentration and effect"
    
  } else if (grepl("^irm_kin_inhibition", x)) {
    desc <- "Indirect response model with inhibition of production rate (KIN)"
    
  } else if (grepl("^irm_kin_stimulation", x)) {
    desc <- "Indirect response model with stimulation of production rate (KIN)"
    
  } else if (grepl("^irm_kout_inhibition", x)) {
    desc <- "Indirect response model with inhibition of loss rate (KOUT)"
    
  } else if (grepl("^irm_kout_stimulation", x)) {
    desc <- "Indirect response model with stimulation of loss rate (KOUT)"
    
  } else if (grepl("^myelosuppression", x)) {
    desc <- "Friberg et al. (2002) semi‑mechanistic myelosuppression model with transit compartments"
    
  } else if (grepl("^transit_cmt", x)) {
    desc <- "Generic transit‑compartment PD model with drug‑dependent inhibition of proliferation"
    
  } else {
    desc <- "Pharmacodynamic model"
  }
...
```{r, results='asis'}
tmdds <- campsismod::model_suite$tmdd
...
```{r, results='asis'}
nonmems <- campsismod::model_suite$nonmem
...
```{r, results='asis'}
pds <- campsismod::model_suite$pd

for (name in names(pds)) {
  model <- pds[[name]]
  description <- getPDDescription(name)
  thetas <- getThetas(model)
  ode <- model %>% find(OdeRecord())
  printModelSummary(description, thetas, ode)
}
