...
In this case, A suffix is automatically added to each model. By default, suffix **A** is assigned to the existing model and **B** to the imported model and the underscore (**_**) is used as a separator. This can be changed by the user.
...
Click the **Download** button to save a JSON file containing the complete Campsis script, including the model code and parameter tables.  ![](pictures/download.png){ style="display: block; margin: 0 auto" }

You can open this JSON file in a text editor to view its contents. The output is shown below:

![](pictures/model.json){width="21cm" height="29cm"}